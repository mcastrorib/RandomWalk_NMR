...
def main():
	...
	for walkers in walker_strings:
		...
		line2 = r'$\rho a/D$= ' + str(relaxation_strength) + r', $D_{p}$ = ' + str(Dp)  + r' $\mu$m²/s, walkers=' + walkers + r', step size = ' + str(step_size) + r' $\mu$m'
		plot_title =  line1 + '\n' + line2
		...
		for row in range(rows):
			for col in range(cols):
				...
				axs[row, col].plot(rw_wrap[idx]['t'], rw_wrap[idx]['D_msd'], 'o', label='msd')
				axs[row, col].plot(rw_wrap[idx]['t'], rw_wrap[idx]['D_sat'], 'x', label='S&T')
				new_title = 'a =' + str(edge_length[idx]) + '$\mu m$, $\phi_{img}$ = ' +  str(true_phi[idx]) 
				axs[row, col].set_title(new_title)
				axs[row, col].set_xlim([0.0, 1.0])
				axs[row, col].set_ylim([0.3, 1.0])	
